...
const state={
  ...
  panels:new Set(DEFAULT_PANELS),view:'fan',boxMode:'separate',range:168,units:'metric',timeMode:'local',threshold:.2,
...
function renderLegend(){const parts=[];if(state.view==='fan'&&state.mode==='forecast')parts.push('<span class="legend-item"><i class="legend-line" style="background:linear-gradient(90deg,#b9c9d4,#547b94)"></i>primary 10–90 / 25–75%</span>');if(state.view==='box'&&state.boxMode==='combined')parts.push('<span class="legend-item"><i class="legend-line" style="background:#007c91"></i>combined boxes · each model weighted equally</span>');for(const m of state.models.values())parts.push(`<span class="legend-item"><i class="legend-line" style="background:${m.color}"></i>${m.name}${m.id===state.primary?' · primary':''}</span>`);$('legend').innerHTML=parts.join('')}
...
function syncControls(){document.querySelectorAll('#models input').forEach(x=>x.checked=state.selected.has(x.value));document.querySelectorAll('[data-view]').forEach(x=>{x.classList.toggle('active',x.dataset.view===state.view);x.setAttribute('aria-pressed',x.dataset.view===state.view)});document.querySelectorAll('[data-range]').forEach(x=>x.classList.toggle('active',Number(x.dataset.range)===state.range));$('box-mode').value=state.boxMode;$('units').value=state.units;$('time-mode').value=state.timeMode;if([...$('rain-threshold').options].some(o=>Number(o.value)===state.threshold))$('rain-threshold').value=state.threshold}
