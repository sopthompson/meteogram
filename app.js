...
function setMapPoint(lat,lon){mapPoint={lat,lon};if(mapMarker)mapMarker.setLatLng([lat,lon]);else mapMarker=L.circleMarker([lat,lon],{radius:8,color:'#fff',weight:2,fillColor:'#65c7e8',fillOpacity:1}).addTo(worldMap);$('map-coordinates').textContent=`${lat.toFixed(4)}°, ${lon.toFixed(4)}°`;$('use-map-location').disabled=false}
