import { MODEL_DEFS,VARIABLE_DEFS,API_VARIABLES,statsAt,circularMeanAt,equalWeightedStats,parsePlaceQuery,normalizeModel,niceBounds,convert,unitFor,formatValue,nearestIndex,dailySummary,solarElevation } from './lib.mjs';
...
const SEASONAL_VARS='temperature_2m,precipitation,wind_speed_10m,wind_direction_10m,cloud_cover,pressure_msl';
const SEASONAL_DEF={id:'ecmwf_seasonal_seamless',name:'ECMWF seasonal seamless',short:'EC46 / SEAS5',members:51,horizon:5136,color:'#76519a'};
...
const state={
  ...
  panels:new Set(DEFAULT_PANELS),view:'fan',fanMode:'members',boxMode:'separate',range:168,units:'metric',timeMode:'local',threshold:.2,
...
function rebuildPrimary(){const select=$('primary');if(state.mode==='history'){select.replaceChildren(new Option('ERA5 reanalysis','era5',true,true));select.disabled=true;return}if(state.mode==='seasonal'){select.replaceChildren(new Option(SEASONAL_DEF.name,SEASONAL_DEF.id,true,true));select.disabled=true;return}const available=MODEL_DEFS.filter(m=>state.selected.has(m.id)&&(!state.models.size||state.models.has(m.id)));select.replaceChildren(...available.map(m=>new Option(m.name,m.id,m.id===state.primary,m.id===state.primary)));select.disabled=available.length<2}
...
async function loadSeasonal(){
  const request=++state.request;state.controller?.abort();state.controller=new AbortController();state.mode='seasonal';state.models.clear();show(els.error,'');show(els.note,'');setLoading('Loading seven-month seasonal ensemble…');
  const params=new URLSearchParams({latitude:state.location.lat,longitude:state.location.lon,hourly:SEASONAL_VARS,models:SEASONAL_DEF.id,forecast_days:'214',wind_speed_unit:'ms',timezone:'GMT'});
  try{const result=await fetchCached(`${SEASONAL_API}?${params}`,`seasonal:v1:${state.location.lat.toFixed(4)},${state.location.lon.toFixed(4)}`,6*60*60*1000,state.controller.signal);if(request!==state.request)return;const model=normalizeModel(result.json,SEASONAL_DEF);state.models.set(model.id,model);state.primary=model.id;state.loadedAt=result.savedAt;state.cacheState=result.source;state.cursor=null;finishLoad();show(els.note,'Seasonal guidance is 6-hourly, 36 km and not bias-corrected. Use it for broad warmer/colder or wetter/drier signals, not precise local weather.')}
  catch(error){if(error.name!=='AbortError'){show(els.error,`Could not load the seasonal forecast: ${error.message}`);els.meta.textContent='Seasonal forecast unavailable'}}
}

function finishLoad(){
  const primary=state.models.get(state.primary),ensembleSize=m=>(m.vars.t2m?.members.length||0)+(m.vars.t2m?.control?1:0);
  els.title.textContent=`${state.mode==='history'?'Weather history':state.mode==='seasonal'?'Seasonal ensemble':'Ensemble meteogram'} — ${state.location.name}`;
...
function visible(){const primary=state.models.get(state.primary);if(!primary)return null;let start=0;if(state.mode==='forecast'){const now=Date.now();start=Math.max(0,primary.times.findIndex(t=>t.getTime()>=now));if(start<0)start=0}const span=state.mode==='forecast'?state.range:primary.times.length,end=Math.min(primary.times.length,start+span+1);return{primary,start,end,times:primary.times.slice(start,end)}}

function render(){if(!state.models.size)return;renderLegend();renderDaily();draw();renderTable()}
function renderLegend(){const parts=[];if(state.view==='fan'&&state.mode!=='history')parts.push(`<span class="legend-item"><i class="legend-line" style="background:linear-gradient(90deg,rgba(0,124,145,.2),#007c91)"></i>${state.fanMode==='members'?'thin lines: individual members · bold line: median':'outer band: 10–90% · inner band: 25–75% · line: median'}</span>`);if(state.view==='box'&&state.boxMode==='combined')parts.push('<span class="legend-item"><i class="legend-line" style="background:#007c91"></i>combined boxes · each model weighted equally</span>');for(const m of state.models.values()){const count=m.vars.t2m?.members.length||0;parts.push(`<span class="legend-item"><i class="legend-line" style="background:${m.color}"></i>${m.name}${state.view==='fan'&&count?` · ${count} members`:''}${m.id===state.primary?' · primary':''}</span>`)}$('legend').innerHTML=parts.join('')}
function renderDaily(){const v=visible();if(!v||state.mode!=='forecast'){ $('daily').replaceChildren();return }const summaries=dailySummary(v.primary,v.start,v.end,state.threshold,timeZone()).slice(0,Math.ceil(state.range/24)+1),threshold=`${formatValue(state.threshold,'precipitation',state.units)}/h`;$('daily').innerHTML=summaries.map(d=>`<article class="day-card"><strong>${formatDate(d.date,{weekday:'short',day:'numeric',month:'short'})}</strong><div class="temp">${formatValue(d.max,'temperature',state.units)} / ${formatValue(d.min,'temperature',state.units)}</div><span>Rain total ${formatValue(d.rain,'precipitation',state.units)}</span><span>Peak chance ≥ ${threshold}: ${d.wet?.toFixed(0)??'—'}%</span><span>Wind ${formatValue(d.wind,'wind',state.units)}${d.gust!=null?` · gust ${formatValue(d.gust,'wind',state.units)}`:''}</span></article>`).join('')}
...
function combinedStats(key,time){const sets=[],directions=[];for(const model of state.models.values()){const variable=model.vars[key];if(!variable?.members.length)continue;const index=nearestIndex(model.times,time.getTime());if(index<0||Math.abs(model.times[index]-time)>3700000)continue;if(key==='direction'){const mean=circularMeanAt(variable.members,index);if(mean!=null)directions.push([mean])}else sets.push(variable.members.map(member=>[member[index]]))}if(key==='direction'){const mean=circularMeanAt(directions,0);return mean==null?null:{min:0,p10:mean,p25:mean,median:mean,p75:mean,p90:mean,max:360,probability:0,count:directions.length}}return sets.length?equalWeightedStats(sets,0,state.threshold):null}
function displayedCombinedStats(key,time){const def=VARIABLE_DEFS.find(v=>v.key===key)||{type:key==='gust'?'wind':''},s=combinedStats(key,time);if(!s)return null;for(const k of ['min','p10','p25','median','p75','p90','max'])s[k]=convert(s[k],def.type,state.units);return s}
...
function draw(){
  ...
  for(const panel of panels){const bot=y0+panel.h;let lo=Infinity,hi=-Infinity;for(const model of state.models.values())for(const time of sampleTimes(v.times,1)){const s=displayedStats(model,panel.key,time);if(s){lo=Math.min(lo,s.min);hi=Math.max(hi,s.max);if(panel.gust){const g=displayedStats(model,panel.gust,time);if(g)hi=Math.max(hi,g.max)}}}if(panel.zero)lo=Math.min(lo,0);const fixed=panel.fixed?.map(n=>convert(n,panel.type,state.units)),bounds=niceBounds(lo,hi,fixed),Y=value=>bot-(value-bounds.lo)/(bounds.hi-bounds.lo)*panel.h;
    ctx.fillStyle='#f8fafb';ctx.fillRect(left,y0,plotW,panel.h);ctx.fillStyle='rgba(0,59,92,.07)';for(const [a,b] of night)ctx.fillRect(a,y0,b-a,panel.h);
    ctx.lineWidth=1;ctx.font='10px Arial,sans-serif';ctx.textAlign='right';for(let g=bounds.lo;g<=bounds.hi+bounds.step/10;g+=bounds.step){const yy=Y(g);ctx.strokeStyle='rgba(29,65,86,.12)';ctx.beginPath();ctx.moveTo(left,yy);ctx.lineTo(left+plotW,yy);ctx.stroke();ctx.fillStyle='#5d7280';ctx.fillText(Math.abs(g)>=10?g.toFixed(0):g.toFixed(1),left-5,yy+3)}
    for(const tick of ticks){ctx.strokeStyle='rgba(29,65,86,.13)';ctx.beginPath();ctx.moveTo(x(tick),y0);ctx.lineTo(x(tick),bot);ctx.stroke()}
    if(state.view==='box'&&state.mode!=='history')drawBoxes(panel,v.times,Y,x,plotW);else drawFan(panel,v.times,Y,x);
    ctx.fillStyle='#203b4d';ctx.font='600 12px Arial,sans-serif';ctx.textAlign='left';ctx.fillText(`${panel.title} (${unitFor(panel.type,state.units)})`,left+6,y0+15);ctx.strokeStyle='rgba(29,65,86,.22)';ctx.strokeRect(left,y0,plotW,panel.h);
    state.layout.panels.push({panel,top:y0,bot,Y,bounds});y0=bot+12;
  }
  ctx.font='11px Arial,sans-serif';ctx.textAlign='center';ctx.fillStyle='#526979';const labelEvery=Math.max(1,Math.ceil(ticks.length/Math.max(1,Math.floor(plotW/60))));for(const [i,tick] of ticks.entries())if(i%labelEvery===0){const xx=x(tick);ctx.fillText(formatDate(tick,{weekday:'short',day:'numeric'}),xx,y0+16)}
...
function memberLine(times,model,member,type,x,Y,color){ctx.beginPath();let moved=false,previous=null;for(const time of times){const index=nearestIndex(model.times,time.getTime()),raw=index>=0&&Math.abs(model.times[index]-time)<=3700000?member[index]:null,value=convert(raw,type,state.units);if(Number.isFinite(value)){const wrapped=type==='direction'&&previous!=null&&Math.abs(value-previous)>180;if(!moved||wrapped)ctx.moveTo(x(time),Y(value));else ctx.lineTo(x(time),Y(value));moved=true;previous=value}else{moved=false;previous=null}}ctx.strokeStyle=color;ctx.lineWidth=.65;ctx.globalAlpha=.2;ctx.stroke();ctx.globalAlpha=1}
function drawFan(panel,times,Y,x){for(const model of state.models.values()){const variable=model.vars[panel.key];if(!variable)continue;if(state.fanMode==='members'){for(const member of variable.members)memberLine(times,model,member,panel.type,x,Y,model.color)}else if(panel.type!=='direction'){band(times,model,panel.key,'p10','p90',x,Y,model.color,'24');band(times,model,panel.key,'p25','p75',x,Y,model.color,'48')}pathLine(times,time=>displayedStats(model,panel.key,time)?.median,x,Y,model.color,2);if(variable.control)pathLine(times,time=>displayedStats(model,panel.key,time)?.control,x,Y,model.color,1.1,[4,3]);if(panel.gust)pathLine(times,time=>displayedStats(model,panel.gust,time)?.median,x,Y,model.color,1,[2,3])}}
function drawBoxes(panel,times,Y,x,plotW){const combined=state.boxMode==='combined',models=combined?[{color:'#007c91'}]:[...state.models.values()],baseHours=state.range<=48?3:state.range<=168?6:12,hours=combined&&state.range>48?Math.max(6,baseHours/2):baseHours,sample=times.filter((_,i)=>i%hours===0),group=Math.min(plotW/sample.length*.82,combined?18:30),bw=Math.max(2,group/models.length-1);for(const time of sample){models.forEach((model,mi)=>{const s=combined?displayedCombinedStats(panel.key,time):displayedStats(model,panel.key,time);if(!s)return;const xx=x(time)-group/2+(mi+.5)*group/models.length;ctx.strokeStyle=model.color;ctx.fillStyle=model.color+'55';ctx.lineWidth=1;if(panel.type==='direction'){ctx.beginPath();ctx.arc(xx,Y(s.median),Math.max(2,bw/2),0,Math.PI*2);ctx.fill();ctx.stroke();return}ctx.beginPath();ctx.moveTo(xx,Y(s.p10));ctx.lineTo(xx,Y(s.p90));ctx.stroke();ctx.fillRect(xx-bw/2,Y(s.p75),bw,Math.max(1,Y(s.p25)-Y(s.p75)));ctx.strokeRect(xx-bw/2,Y(s.p75),bw,Math.max(1,Y(s.p25)-Y(s.p75)));ctx.beginPath();ctx.moveTo(xx-bw/2,Y(s.median));ctx.lineTo(xx+bw/2,Y(s.median));ctx.stroke();ctx.beginPath();ctx.moveTo(xx-bw/3,Y(s.p10));ctx.lineTo(xx+bw/3,Y(s.p10));ctx.moveTo(xx-bw/3,Y(s.p90));ctx.lineTo(xx+bw/3,Y(s.p90));ctx.stroke()})}}
...
function drawCursor(time){const l=state.layout,xx=l.x(new Date(time));ctx.strokeStyle='rgba(184,111,0,.95)';ctx.lineWidth=1.3;ctx.beginPath();ctx.moveTo(xx,l.panels[0]?.top||0);ctx.lineTo(xx,l.panels.at(-1)?.bot||cv.height);ctx.stroke()}
...
async function chooseLocation(location){state.location=location;state.recent=[location,...state.recent.filter(item=>!sameLoc(item,location))].slice(0,15);writeJSON('meteogram.recent',state.recent);rebuildLocations();await loadForecast()}
async function search(){const q=$('q').value.trim();if(!q)return;show(els.error,'');$('go').disabled=true;try{const parsed=parsePlaceQuery(q),params=new URLSearchParams({name:parsed.name,count:'10',language:'en',format:'json'});if(parsed.countryCode)params.set('countryCode',parsed.countryCode);const result=await fetchCached(`${GEO}?${params}`,`geo:v4:${parsed.name.toLowerCase()}:${parsed.countryCode||'*'}`,30*864e5);const places=(result.json.results||[]).map(x=>({name:[x.name,x.admin1,x.country].filter(Boolean).join(', '),detail:[x.admin2,Array.isArray(x.postcodes)?x.postcodes[0]:x.postcodes,x.country_code].filter(Boolean).join(' · '),lat:x.latitude,lon:x.longitude,timezone:x.timezone}));if(!places.length)return show(els.error,`No place found for “${q}”. Try the place name followed by a country, such as “Southport UK”.`);const list=$('search-results');list.replaceChildren(...places.map(place=>{const button=document.createElement('button');button.type='button';button.className='place-result';const name=document.createElement('strong'),detail=document.createElement('span'),coords=document.createElement('span');name.textContent=place.name;detail.textContent=place.detail||'Select this result';coords.className='coords';coords.textContent=`${place.lat.toFixed(3)}°, ${place.lon.toFixed(3)}°`;button.append(name,detail,coords);button.addEventListener('click',()=>{$('search-dialog').close();chooseLocation(place)});return button}));$('search-dialog').showModal()}catch(error){show(els.error,`Search failed: ${error.message}`)}finally{$('go').disabled=false}}
...
let worldMap=null,mapMarker=null,mapPoint=null;
function openMap(){if(!window.L)return show(els.error,'The map library could not load. Check your connection and try again.');$('map-dialog').showModal();if(!worldMap){worldMap=L.map('world-map',{worldCopyJump:true}).setView([state.location.lat,state.location.lon],Math.abs(state.location.lat)>70?3:5);L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'}).addTo(worldMap);worldMap.on('click',event=>setMapPoint(event.latlng.lat,event.latlng.lng))}else worldMap.setView([state.location.lat,state.location.lon],worldMap.getZoom());setTimeout(()=>worldMap.invalidateSize(),0)}
function setMapPoint(lat,lon){mapPoint={lat,lon};if(mapMarker)mapMarker.setLatLng([lat,lon]);else mapMarker=L.circleMarker([lat,lon],{radius:8,color:'#fff',weight:2,fillColor:'#007c91',fillOpacity:1}).addTo(worldMap);$('map-coordinates').textContent=`${lat.toFixed(4)}°, ${lon.toFixed(4)}°`;$('use-map-location').disabled=false}
async function useMapLocation(){if(!mapPoint)return;const {lat,lon}=mapPoint;$('use-map-location').disabled=true;$('map-coordinates').textContent='Resolving local time zone…';let timezone='UTC';try{const params=new URLSearchParams({latitude:lat,longitude:lon,current:'temperature_2m',forecast_days:'1',timezone:'auto'}),result=await fetchCached(`${FORECAST}?${params}`,`timezone:v1:${lat.toFixed(2)},${lon.toFixed(2)}`,30*864e5);timezone=result.json.timezone||'UTC'}catch{}$('map-dialog').close();await chooseLocation({name:`Map point ${lat.toFixed(3)}°, ${lon.toFixed(3)}°`,lat,lon,timezone});$('use-map-location').disabled=false}

function updateUrl(){if(!state.location)return;const p=new URLSearchParams({lat:state.location.lat.toFixed(4),lon:state.location.lon.toFixed(4),name:state.location.name,tz:state.location.timezone||'',models:[...state.selected].join(','),primary:state.mode==='forecast'?state.primary:[...state.selected][0],mode:state.mode==='seasonal'?'seasonal':'forecast',view:state.view,fan:state.fanMode,boxes:state.boxMode,range:state.range,units:state.units,time:state.timeMode,rain:state.threshold,panels:[...state.panels].join(',')});history.replaceState(null,'',`${location.pathname}?${p}`)}
function loadUrl(){const p=new URLSearchParams(location.search),lat=Number(p.get('lat')),lon=Number(p.get('lon'));if(p.has('lat')&&p.has('lon')&&Number.isFinite(lat)&&Number.isFinite(lon))state.location={name:p.get('name')||`${lat.toFixed(2)}, ${lon.toFixed(2)}`,lat,lon,timezone:p.get('tz')||Intl.DateTimeFormat().resolvedOptions().timeZone};else state.location=readJSON('meteogram.last',DEFAULT_LOC);const ids=(p.get('models')||'').split(',').filter(id=>MODEL_DEFS.some(m=>m.id===id));if(ids.length)state.selected=new Set(ids);if(p.get('mode')==='seasonal')state.mode='seasonal';if(['fan','box'].includes(p.get('view')))state.view=p.get('view');if(['members','percentiles'].includes(p.get('fan')))state.fanMode=p.get('fan');if(['separate','combined'].includes(p.get('boxes')))state.boxMode=p.get('boxes');if([48,168,360].includes(Number(p.get('range'))))state.range=Number(p.get('range'));if(['metric','uk','us'].includes(p.get('units')))state.units=p.get('units');if(['local','utc'].includes(p.get('time')))state.timeMode=p.get('time');if(Number.isFinite(Number(p.get('rain')))&&Number(p.get('rain'))>0)state.threshold=Number(p.get('rain'));const panels=(p.get('panels')||'').split(',').filter(key=>VARIABLE_DEFS.some(v=>v.key===key));if(panels.length)state.panels=new Set(panels);state.primary=state.selected.has(p.get('primary'))?p.get('primary'):[...state.selected][0]}
...
function bind(){
  $('go').addEventListener('click',search);$('q').addEventListener('keydown',e=>{if(e.key==='Enter')search()});$('near').addEventListener('click',useLocation);$('open-map').addEventListener('click',openMap);$('use-map-location').addEventListener('click',useMapLocation);els.loc.addEventListener('change',async()=>{state.location=els.loc._locations[Number(els.loc.value)];await loadForecast()});
  ...
  $('fan-mode').addEventListener('change',()=>{state.fanMode=$('fan-mode').value;updateUrl();render()});
  $('box-mode').addEventListener('change',()=>{state.boxMode=$('box-mode').value;updateUrl();render()});
  ...
  $('history').addEventListener('click',loadHistory);$('seasonal').addEventListener('click',loadSeasonal);$('forecast').addEventListener('click',loadForecast);$('share').addEventListener('click',share);$('csv').addEventListener('click',exportCsv);$('png').addEventListener('click',exportPng);
...
function syncControls(){document.querySelectorAll('#models input').forEach(x=>x.checked=state.selected.has(x.value));document.querySelectorAll('[data-view]').forEach(x=>{x.classList.toggle('active',x.dataset.view===state.view);x.setAttribute('aria-pressed',x.dataset.view===state.view)});document.querySelectorAll('[data-range]').forEach(x=>x.classList.toggle('active',Number(x.dataset.range)===state.range));$('fan-mode').value=state.fanMode;$('box-mode').value=state.boxMode;$('units').value=state.units;$('time-mode').value=state.timeMode;if([...$('rain-threshold').options].some(o=>Number(o.value)===state.threshold))$('rain-threshold').value=state.threshold}

loadUrl();buildControls();syncControls();state.recent=[state.location,...state.recent.filter(x=>!sameLoc(x,state.location))].slice(0,15);rebuildLocations();initDates();bind();state.mode==='seasonal'?loadSeasonal():loadForecast();
